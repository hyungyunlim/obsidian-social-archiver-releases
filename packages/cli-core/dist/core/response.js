 ...
 */
export const RETRYABLE_BY_CODE = Object.freeze({
 ...
 */
export const BILLING_FALLBACK_MESSAGE = 'Insufficient credits. Upgrade or restore via the mobile app using the same account, or apply a license key in plugin settings.';
/**
 * Build a success envelope. Caller passes the current host version so this
 * module stays decoupled from any specific host.
 */
export function ok(command, version, data, opts = {}) {
    const envelope = {
        ok: true,
        command,
        version,
 ...
 */
export function err(command, version, code, message, opts = {}) {
    const known = RETRYABLE_BY_CODE[code];
...
function isSensitiveKey(key) {
    ...
    for (const suffix of SENSITIVE_KEY_SUFFIXES) {
        ...
        if (lower.length > suffixLower.length && lower.endsWith(suffixLower)) {
            ...
            const here = key[idx];
            if (prev === undefined || here === undefined)
                continue;
            if (prev === '_' || prev === '-' || prev === '.')
                return true;
            if (prev >= 'a' && prev <= 'z' && here >= 'A' && here <= 'Z')
                return true;
            if (lower === suffixLower)
                return true;
        }
    }
    return false;
...
const PREFIXED_UUID_PATTERN = /^[A-Za-z][A-Za-z0-9]*[._-][0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const HEX_DIGEST_PATTERN = /^[0-9a-f]+$/i;
function isStandardIdentifier(token) {
...
function redactString(input, opts = {}) {
    ...
    out = out.replace(OPAQUE_TOKEN_PATTERN, (match) => {
        if (match === REDACTED_BARE)
            return match;
        if (isStandardIdentifier(match))
            return match;
        return REDACTED;
    });
    return out;
 ...
 */
export function format(response, mode = 'json') {
    if (mode === 'text')
