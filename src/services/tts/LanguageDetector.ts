...
const DIACRITIC_LANGUAGE_MARKERS: Array<{ lang: string; chars: Set<number>; threshold: number }> = [
...
function detectLatinLanguage(sample: string, total: number): string {
  ...
  if (stopWordMatch) return stopWordMatch;
 ...
 */
function detectByStopWords(sample: string): string | undefined {
...
function detectByDiacriticalMarkers(sample: string, total: number): string | undefined {
  for (const marker of DIACRITIC_LANGUAGE_MARKERS) {
    let count = 0;
    for (let i = 0; i < sample.length; i++) {
      if (marker.chars.has(sample.charCodeAt(i))) count++;
    }
...
function detectCyrillicLanguage(sample: string): string {
  if (hasUkrainianSpecificChars(sample)) return 'uk-UA';
...
function hasUkrainianSpecificChars(sample: string): boolean {
  for (let i = 0; i < sample.length; i++) {
    const code = sample.charCodeAt(i);
    if (
      code === 0x0404 || code === 0x0454 || // Є є
...
export function detectLanguage(text: string): string | undefined {
  ...
  if (greekRatio >= GREEK_THRESHOLD) return 'el-GR';
  // Cyrillic: distinguish Bulgarian/Ukrainian where possible; default to Russian.
  if (cyrillicRatio >= CYRILLIC_THRESHOLD) return detectCyrillicLanguage(sample);
