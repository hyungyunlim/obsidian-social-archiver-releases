 ...
 */
export function extractTextFromMarkdown(rawContent) {
...
export function extractText(post) {
    const rawText = (post.fullContent ?? '').trim() ||
 ...
 */
export function buildOffsetMap(rawText, cleanedText) {
    if (!cleanedText || !rawText)
        return null;
    const map = new Array(cleanedText.length + 1);
    let fi = 0;
    ...
    for (let ci = 0; ci < cleanedText.length; ci++) {
        ...
        if (cc === ' ') {
            while (fi < rawText.length && !/\s/.test(rawText[fi] ?? ''))
                fi++;
 ...
 */
export function isSpeakable(text, minWords = MIN_SPEAKABLE_WORDS) {
    return countWords(text) >= minWords;
}
//# sourceMappingURL=TTSTextProcessor.js.map