...
export class ArchiveProgressBanner {
  ...
  private addRetryButton(actionsEl: HTMLElement, jobId: string): void {
    const retryBtn = actionsEl.createEl('button', {
      cls: 'banner-retry clickable-icon apb-action-btn',
  ...
  private addDismissButton(actionsEl: HTMLElement, jobId: string): void {
    const dismissBtn = actionsEl.createEl('button', {
      cls: 'banner-dismiss clickable-icon apb-action-btn',
   ...
   */
  private applyIconStatusClass(iconEl: HTMLElement, status: ActiveArchiveJob['status']): void {
    // Remove all status classes
    iconEl.removeClass('apb-icon-queued', 'apb-icon-active', 'apb-icon-completed', 'apb-icon-failed');

    if (status === 'queued') {
      iconEl.addClass('apb-icon-queued');
    } else if (status === 'archiving' || status === 'retrying') {
      iconEl.addClass('apb-icon-active');
    } else if (status === 'completed') {
      iconEl.addClass('apb-icon-completed');
    } else if (status === 'failed') {
      iconEl.addClass('apb-icon-failed');
    }
  }
